...
jQuery(function ($) {
        $('div').on('click', '.img-wrap .close', function () {
            ...
            var indexOfPush = arrPush.map(function (x) { return x.name; }).indexOf(id);
            arrPush.forEach(function(x){
                if(x.name == id){
                    delete x;
                }
            });
            if (indexOfPush !== -1) {
                arrPush.splice(indexOfPush, 1);
            }
...
//Render attachments thumbnails.
function RenderThumbnail(e, readerEvt,url = null)
{
    // console.log(url);
    if(url==null){
        url = e.target.result;
    ...
    '<img class="thumb" src="', url, '" title="', escape(name), '" data-id="',
        name, '"/>' + '</div>'].join('');

    var div = document.createElement('div');
    div.className = "FileNameCaptionStyle";
    li.appendChild(div);
    // div.innerHTML = [name].join('');
